...
def render_camp_overview():
    ...
    with st.expander("📊 만족도 조사 설문 링크"):
        st.markdown("[🔗 캠프 만족도 조사 참여하기](#)")
...
if not st.session_state.logged_in:
    ...
    # [화면 B] 메인 대시보드
        ...
        # --------- 관리자/교사 ---------
            ...
            menu_tabs = st.tabs(["📌 캠프 공지(미리보기)", "👥 회원 관리", "🗂️ 차시 편집", "📥 학생 제출 자료 조회(다운로드)"]) if current_role == "관리자" else st.tabs(["📌 캠프 공지(미리보기)", "👥 회원 관리", "📥 학생 제출 자료 조회(다운로드)"])
            ...
            if current_role == "관리자":
                with menu_tabs[2]:
                    ...
                    with col1:
                        ...
                        if st.button("차시 개설하기"):
                            if new_tab_name and new_tab_name not in app_config["tabs"]:
                                app_config["tabs"].append(new_tab_name)
                                app_config["pdfs"][new_tab_name] = new_pdf_name
                                app_config["questions"][new_tab_name] = []
                                save_json(CONFIG_FILE, app_config)
                                st.success(f"🎉 {new_tab_name}이 정상 개설되었습니다.")
                                st.rerun()
                            else:
                                st.error("입력값이 올바르지 않거나 이미 존재하는 차시명입니다.")
                    ...
                    with col2:
                        ...
                        if del_tab_target != "선택":
                            if st.button(f"🔥 {del_tab_target} 세션 및 질문 전체 삭제", type="primary"):
                                app_config["tabs"].remove(del_tab_target)
                                app_config["pdfs"].pop(del_tab_target, None)
                                app_config["questions"].pop(del_tab_target, None)
                                save_json(CONFIG_FILE, app_config)
                                st.success(f"{del_tab_target} 차시가 정상 삭제되었습니다.")
                                st.rerun()
                    ...
                    if target_q_tab:
                        st.write(f"현재 **{target_q_tab}**에 배치된 문항들:")
                        current_qs = app_config["questions"].get(target_q_tab, [])
                        for q in current_qs:
                            st.text(f" - [{q['id']}] {q['label']}")
                            
                        q_col1, q_col2 = st.columns(2)
                        with q_col1:
                            st.write("➕ **질문 항목 추가**")
                            add_q_label = st.text_input("질문 설명(라벨) 문구를 적으세요", placeholder="예: 탐구 주제를 입력하세요.")
                            if st.button("질문 추가"):
                                if add_q_label:
                        ...
                        with q_col2:
                            ...
                            if current_qs:
                                ...
                                if st.button("선택한 문항 삭제", type="primary"):
                                    current_qs.remove(del_q_target)
                                    app_config["questions"][target_q_tab] = current_qs
                                    save_json(CONFIG_FILE, app_config)
                                    st.success("문항이 삭제되었습니다.")
                                    st.rerun()
                            else:
                                st.info("삭제할 문항이 존재하지 않습니다.")
                ...
                if not student_list: st.info("가입된 학생이 없습니다.")
                    ...
                    if view_mode == "👤 특정 학생 집중 분석":
                        ...
                        if selected_view in ACTIVITIES:
                            ...
                            st.download_button(f"📊 엑셀(CSV) 다운로드", data=csv, file_name=f"{selected_view}_결과.csv", mime='text/csv')
